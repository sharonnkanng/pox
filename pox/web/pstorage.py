    ...
    if m.is_multipart():
      for part in m.walk():
        if not part.is_multipart():
          f = FieldStorage()
          # The official document says `get_param()` and `get_payload()` are legacy methods.
          # However, all value retrieval functions within the source code are utilizing these methods.
